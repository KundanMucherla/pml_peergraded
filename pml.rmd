...
```{r}
par <- read.csv("pml-training.csv", stringsAsFactors = F,na.strings = c("","NA","#DIV/0!"))
sec_par <- read.csv("pml-testing.csv", stringsAsFactors = F,na.strings = c("","NA","#DIV/0!"))
```

```{r}
partition <- createDataPartition(par$classe, p = 0.8, list = F)
ger <- par[-partition,]
par <- par[partition,]
```

```{r}
kundan <- sapply(select(par,names(par)[grepl("_belt",names(par))]),
                    function(x) sum(is.na(x)))
```
```{r}
arm_m <- sapply(select(par,names(par)[grepl("_arm",names(par))]),
                   function(x) sum(is.na(x)))
```
```{r}
f_m <- sapply(select(par,
                              names(par)[grepl("_forearm",names(par))]),
                       function(x) sum(is.na(x)))
```

```{r}
dumkundan <- sapply(select(par,
                               names(par)[grepl("_dumbbell",names(par))]),
                        function(x) sum(is.na(x)))
```

```{r}
c_d <- c(names(kundan[kundan != 0]), 
                  names(arm_m[arm_m != 0]),
                  names(f_m[f_m != 0]),
                  names(dumkundan[dumkundan != 0]))
```
```{r}
a_n_d <- tbl_df(par %>% 
                      select(-c_d,
...
```{r, echo=FALSE, eval=FALSE}
daki <- head(corritmonos %>% mutate(corr = abs(correlation)) %>% 
  arrange(desc(corr)) %>% select(feature) %>% unique(),20)
daki$feature[which(daki$feature %in% craasd)]
```
